...
# 1. 定义与训练时一致的模型结构（双层 LSTM + Dropout）
class LSTMModel(torch.nn.Module):
    def __init__(self, input_size=8, hidden_size_1=128, hidden_size_2=64, output_size=1, dropout=0.2):
        super(LSTMModel, self).__init__()
        self.lstm1 = torch.nn.LSTM(input_size, hidden_size_1, batch_first=True)
        self.dropout1 = torch.nn.Dropout(dropout)
        self.lstm2 = torch.nn.LSTM(hidden_size_1, hidden_size_2, batch_first=True)
        self.dropout2 = torch.nn.Dropout(dropout)
        self.fc = torch.nn.Linear(hidden_size_2, output_size)
    ...
    def forward(self, x):
        ...
        return self.fc(out[:, -1, :])  # 使用最后一个时间步输出
...
def predict_headroom(data_dict):
    ...
    X_tensor = torch.tensor(X_seq[-seq_length:], dtype=torch.float32).unsqueeze(0)  # shape: (1, seq_length, 8)
...
while True:
    ...
    while True:
        try:
            ...
            # 返回结果
            response = f"{headroom:.2f}\n"
            conn.sendall(response.encode('utf-8'))
            print(f"发送预测值: {response.strip()}")
