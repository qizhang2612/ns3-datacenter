...
# ) -> str:
#     test_name = f'exp_benchmark_dt-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg'
...
#     copy_src = f'./data/exp_benchmark_dt-search-xpod_Normal-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
#     copy_dst = f'./data/exp_benchmark_dt-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
...
def exp_benchmark_fct(  # pylint: disable=W0102
    ...
    test_name = f'exp_benchmark_fct-dt-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg'
    test_dir = utils.get_test_dir(test_name)
    test_dir.mkdir(exist_ok=True)
    ...
    #control the same flow between SIH and AISIH
    copy_src = f'./data/exp_benchmark_fct-dt-search-xpod_Normal-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    copy_dst = f'./data/exp_benchmark_fct-dt-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    if mmu_kind != 'Normal' and os.path.exists(copy_src):
        os.system('cp -rf %s %s'%(copy_src, copy_dst))
    else:
        flows: FlowList = []
