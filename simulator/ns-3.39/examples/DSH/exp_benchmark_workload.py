...
def exp_benchmark_fct(  # pylint: disable=W0102
    ...
    oversubratio = nhost / nspine
    test_name = f'exp_benchmark_fct-{back_pattern}-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg'
    ...
    copy_src = f'./data/exp_benchmark_fct-{back_pattern}-xpod_Normal-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    copy_dst = f'./data/exp_benchmark_fct-{back_pattern}-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    if mmu_kind != 'Normal' and os.path.exists(copy_src):
        ...
        # background flow
        flowsz = flow.FlowSizeRng.from_tcl('./flow-cdf/' + back_pattern + '_cdf.tcl')
    ...
    with open(test_dir / 'config.txt', 'w') as configf:
        config.gen(test_name,
                   ...
                   stop_time=(start_time + 20000000) / 1e6,
                   pg_count=mx_pg+1,
                   sche_alg=sche,
                   quantums=quantums,
                   mmu_kind=mmu_kind,
...
def main() -> None:
    tests: Set[str] = set()
    # for pattern in ['hadoop', 'mining', 'cache', 'alistorage']:
    for pattern in ['hadoop', 'mining']:
        for load10 in range(1, 9):
            ...
            for cc in ['DCQCN']:
                for pg in [8]:
                    ...
                    #for mmu in ['Normal','Adaptive', 'DSHPLUS', 'Normal50', 'Normal80']:
                    for mmu in ['DSHPLUS']:
                        # for mmu in ['DH', 'DH3']:
                        ks : List[Any]
