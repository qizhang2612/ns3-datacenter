...
for i in range(0, 11):
    ...
    filename = 'data/exp_pfc_avoidance_DSH-DWRR-None-16h-{:.1f}bp-8pg/pfc.txt'.format(load)
    with open(filename, 'r') as file:  
        for line in file:    
            ...
            if(pause == 0):
                # if(dsh_pause[node][port][queue] == 1 and node >= 2 and node <= 17):
                if(dsh_pause[node][port][queue] == 1 and node > 1):
                    pfc_time += (time - dsh_time[node][port][queue])
...
plt.plot(x2_values, y2_values, marker='x', markersize=15,linestyle = '--', color='g',linewidth=3,label='DSHPLUS') 
