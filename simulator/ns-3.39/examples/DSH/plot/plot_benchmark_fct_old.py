    ...
    for kind in ['back', 'burst']:
        ...
        for cc_kind in ['DCQCN', 'PowerTCP']:
            ...
            for back_load in back_loads:
                ...
                filename2 = f'data/exp_benchmark_fct-{load_kind}-xpod_DSHnoIH-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-64KB-8pg/fct.txt'
                print(filename1)
                back_values = []
                burst_values = []
                back2_values = []
            ...
            if(kind == 'burst'):
                for i in range(1,8):
                    ...
                    y2_values.append(normal_burst_avgs[i-1]/ 1e6)
            ...
            if(kind == 'back'):
                for i in range(1,8):
                    ...
                    y2_values.append(normal_back_avgs[i-1]/ 1e6)
            ...
            plt.plot(x2_values, y2_values, marker='x',markersize=15,linestyle = '--', color='g',linewidth=3,label='DSHnoIH')  
            
            # 设置坐标轴范围（如果需要）  
            # plt.xlim(0.0, 1.0)  
            plt.ylim(0.0, 16.0)  # 增加10%的裕量  
            ...
            # 将图形保存为PDF文件  f
            plt.savefig(f'image/benchmark_noIH_{load_kind}_{kind}_{cc_kind}.png', format='png')  
