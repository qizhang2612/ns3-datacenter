...
# back_loads = [0.9]
#for load_kind in['search','hadoop','cache','mining', 'alistorage']:
for load_kind in['search']:
    for kind in ['back', 'burst']:
        #for cc_kind in ['None', 'DCQCN', 'PowerTCP','HPCC']: 
        for cc_kind in ['None', 'DCQCN', 'PowerTCP','HPCC']:
            x1_values = []
            x2_values = []
            y1_values = []  
            ...
            for back_load in back_loads:
                ...
                filename1 = f'data/exp_benchmark_fct-{load_kind}-xpod_DSHPLUS-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-64KB-8pg/fct.txt'
                filename2 = f'data/exp_benchmark_fct-{load_kind}-xpod_DSH-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-64KB-8pg/fct.txt'
                # filename1 = f'data/exp_benchmark_higburst-{load_kind}-xpod_DSHPLUS-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-128bursthosts-64KB-8pg/fct.txt'
                # filename2 = f'data/exp_benchmark_higburst-{load_kind}-xpod_DSH-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-128bursthosts-64KB-8pg/fct.txt'
                print(filename1)
                back_values = []
                burst_values = []
                back2_values = []
            ...
            if(kind == 'burst'):
                for i in range(1,8):
                    ...
                    print((normal_burst_avgs[i-1] - dsh_burst_avgs[i-1]) / normal_burst_avgs[i-1])
            ...
            if(kind == 'back'):
                for i in range(1,8):
                    ...
                    print((normal_back_avgs[i-1] - dsh_back_avgs[i-1]) / normal_back_avgs[i-1])
                    y2_values.append(1.0)
            ...
            plt.plot(x2_values, y2_values, marker='x',markersize=15,linestyle = '--', color='g',linewidth=3,label='DSH')  
            ...
            # 将图形保存为PDF文件  f
            plt.savefig(f'image/test_dshplus_{load_kind}_{kind}_{cc_kind}_128bursthosts.png', format='png')  
