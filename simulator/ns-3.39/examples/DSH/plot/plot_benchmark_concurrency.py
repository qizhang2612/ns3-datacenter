...
def precompute_intersections():
    ...
    for cc_kind in CC_KINDS:
        for back_load in BACK_LOADS:
            ...
            # 生成文件路径
            file_50 = f'data-final/benchmark_concurrency/exp_benchmark_concurrency-search-xpod_Normal50-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-64KB-8pg/fct.txt'
            file_80 = f'data-final/benchmark_concurrency/exp_benchmark_concurrency-search-xpod_Normal80-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-64KB-8pg/fct.txt'
            # 读取文件数据
...
def process_workload(cc_kind, policy):
    ...
    for back_load in BACK_LOADS:
        ...
        # 生成文件路径
        if(policy == "DSHPLUS"):
            policy_file = f'data-DSH60000/benchmark_concurrency/exp_benchmark_concurrency-search-xpod_{policy}-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-64KB-8pg/fct.txt'
        else:
            policy_file = f'data-final/benchmark_concurrency/exp_benchmark_concurrency-search-xpod_{policy}-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-64KB-8pg/fct.txt'
        base_file = f'data-final/benchmark_concurrency/exp_benchmark_concurrency-search-xpod_Normal-DWRR-{cc_kind}-{back_load}back-{burst_load}burst-64KB-8pg/fct.txt'
        # 读取数据并计算比率
        print(policy_file)
...
def plot_figure(cc_kind, traffic_kind, plot_data):
    ...
    plt.tight_layout()
    plt.savefig(f'image/benchmark_concurrency_search_{traffic_kind}_{cc_kind}.png', dpi=300)
    plt.close()
