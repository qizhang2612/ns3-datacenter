...
def exp_benchmark_vs(  # pylint: disable=W0102
        nspine=16,
        nleaf=16,
        nhost=16,
        SL_Gbps=100,
    ...
    test_name = f'exp_benchmark_vs-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg_{buffer_size}buffer'
    ...
    copy_src = f'./data/exp_benchmark_vs-search-xpod_Normal-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg_{buffer_size}buffer/flow.txt'
    copy_dst = f'./data/exp_benchmark_vs-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg_{buffer_size}buffer/flow.txt'
        ...
        for load10 in range(1, 9):
            ...
            for cc in ['DCQCN','PowerTCP']:
                for pg in [8]:
                    ...
                    #for mmu in ['Normal','DSH', 'Adaptive', 'DSHPLUS', 'Normal50', 'Normal80', 'DSHnoSH', 'DSHnoIH']:
