...
# this script is for easy topo test, and its hostNum = 8
...
def exp_pfc_avoidance(  # pylint: disable=W0102
    ...
    receiver_hosts = burst_hosts # for each pair is (2senders, 1receiver-2~7, 1 receiver-1)

    # topo
    all_nodes = list(range(burst_hosts + receiver_hosts + 1))
    hosts, sws = all_nodes[ : burst_hosts + receiver_hosts], all_nodes[burst_hosts + receiver_hosts : ]
    ...
    for host in all_nodes[ : burst_hosts + receiver_hosts]:
    ...
    for i in range(burst_hosts + receiver_hosts):
        tmpl = []
        tmpl.append(link[i])
            ...
            if (index % 2 == 0):
                ...
                if lam_1 > 0:
                    ...
                    for i in range(len(send_times_1)):
                        ...
                        fsz = flowsz.rand()
                        # fsz = burst_size
                        flows += flow.gen_flows(100, fsz, [(host, hosts[burst_hosts + index], QoS_1, stime)]) # Default  burstTotalSize  srcHost  dstHost  QoS  beginTime
                        # for flow bak. of the next one
                        bakFlow1 += flow.gen_flows(100, fsz, [(host + 1, hosts[burst_hosts + index], QoS_1, stime)]) # a copy of flow (CoS1)
                ...
                if lam_2 > 0:
                    ...
                    for i in range(len(send_times_2)):
                        ...
                        # fsz = flowsz.rand()
                        fsz = burst_size
                        flows += flow.gen_flows(100, fsz, [(host, hosts[burst_hosts + index + 1], QoS_2, stime)])
                        # for flow bak. of the next one
                        bakFlow2 += flow.gen_flows(100, fsz, [(host + 1, hosts[burst_hosts + index + 1], QoS_2, stime)]) # a copy of flow (CoS2)
                ...
                if lam_3 > 0:
                    ...
                    for i in range(len(send_times_3)):
                        ...
                        # fsz = flowsz.rand()
                        fsz = burst_size
                        flows += flow.gen_flows(100, fsz, [(host, hosts[burst_hosts + index + 1], QoS_3, stime)])
                        # for flow bak. of the next one
                        bakFlow3 += flow.gen_flows(100, fsz, [(host + 1, hosts[burst_hosts + index + 1], QoS_3, stime)]) # a copy of flow (CoS3)
                ...
                if lam_4 > 0:
                    ...
                    for i in range(len(send_times_4)):
                        ...
                        # fsz = flowsz.rand()
                        fsz = burst_size
                        flows += flow.gen_flows(100, fsz, [(host, hosts[burst_hosts + index + 1], QoS_4, stime)])
                        # for flow bak. of the next one
                        bakFlow4 += flow.gen_flows(100, fsz, [(host + 1, hosts[burst_hosts + index + 1], QoS_4, stime)]) # a copy of flow (CoS4)
                ...
                if lam_5 > 0:
                    ...
                    for i in range(len(send_times_5)):
                        ...
                        # fsz = flowsz.rand()
                        fsz = burst_size
                        flows += flow.gen_flows(100, fsz, [(host, hosts[burst_hosts + index + 1], QoS_5, stime)])
                        # for flow bak. of the next one
                        bakFlow5 += flow.gen_flows(100, fsz, [(host + 1, hosts[burst_hosts + index + 1], QoS_5, stime)]) # a copy of flow (CoS5)
                ...
                if lam_6 > 0:
                    ...
                    for i in range(len(send_times_6)):
                        ...
                        # fsz = flowsz.rand()
                        fsz = burst_size
                        flows += flow.gen_flows(100, fsz, [(host, hosts[burst_hosts + index + 1], QoS_6, stime)])
                        # for flow bak. of the next one
                        bakFlow6 += flow.gen_flows(100, fsz, [(host + 1, hosts[burst_hosts + index + 1], QoS_6, stime)]) # a copy of flow (CoS6)
                ...
                if lam_7 > 0:
                    ...
                    for i in range(len(send_times_7)):
                        ...
                        # fsz = flowsz.rand()
                        fsz = burst_size
                        flows += flow.gen_flows(100, fsz, [(host, hosts[burst_hosts + index + 1], QoS_7, stime)])
                        # for flow bak. of the next one
                        bakFlow7 += flow.gen_flows(100, fsz, [(host + 1, hosts[burst_hosts + index + 1], QoS_7, stime)]) # a copy of flow (CoS7)
            else:
                flows += bakFlow1
                flows += bakFlow2
                flows += bakFlow3
    ...
    with open(test_dir / 'config.txt', 'w') as configf:
        config.gen(test_name,
                   ...
                   dt_alpha=4, # Default: dt_alpha=4 in dsh-test.cc, you can adjust it here if needed
                   pg_count=mx_pg+1,
                   sche_alg=sche,
                   quantums=quantums,
                   mmu_kind=mmu_kind,
