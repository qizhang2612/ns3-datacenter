...
def exp_benchmark_concurrency(  # pylint: disable=W0102
        nspine=16,
        nleaf=16,
        nhost=16,
        SL_Gbps=100,
    ...
    test_name = f'exp_benchmark_concurrency-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg'
    ...
    copy_src = f'./data/exp_benchmark_concurrency-search-xpod_Normal-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    copy_dst = f'./data/exp_benchmark_concurrency-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    if mmu_kind != 'Normal' and os.path.exists(copy_src):
        ...
        # 调整后的λ计算
        concurrency_nflows = 8
        original_lam = back_load * (LH_Gbps * 1000 / 8) / flowsz.avg()
        adjusted_lam = original_lam / concurrency_nflows  # 关键调整点

        if adjusted_lam > 0:
            for src in hosts:
                # 调整总事件次数（若需保持总流数不变）
                adjusted_flow_num = flow_num // (nhost * nleaf)
                send_times = exp_send_time(adjusted_lam, adjusted_flow_num, seed=datetime.now().timestamp())
                ...
                for i in range(len(send_times)):
                    ...
                    if stime > flow_stop_time:
                        break
                    # 生成8条并发流
                    for _ in range(concurrency_nflows):
                        fsz = flowsz.rand()
            ...
            for pg in [8]:
                ...
                for mmu in ['DSHPLUS']:
                    ...
                    if mmu == 'Normal' or mmu == 'DSH' or mmu == 'DSHnoSH' or mmu == 'DSHnoIH' or mmu == 'Adaptive' or mmu == 'DSHPLUS' or mmu == 'Normal50' or mmu == 'Normal80':
                        ...
                        ks = [pg+1]
                    for k in ks:
                        test_name = exp_benchmark_concurrency(
                            cc=cc,
                            mx_pg=7,
                            mmu_kind=mmu,
                            port_hdrm_k=k,
