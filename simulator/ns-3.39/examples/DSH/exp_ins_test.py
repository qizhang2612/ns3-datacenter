...
def exp_ins_test(  # pylint: disable=W0102
        nspine=0,
        nleaf=1,
        nhost=32,
        SL_Gbps=100,
    ...
    test_name = f'exp_ins_test_{mmu_kind}-{sche}-{cc}-{burst_hosts}h-{mx_pg+1}pg'
    ...
    for i in range(0, 31):
        for j in range(1, 8):
            flows += flow.gen_flows(100, 250000, [(host[i], host[-1], j, start_time)])

    # for i in range(1, 3):
    #     # for j in range(1, 2):
    #         flows += flow.gen_flows(100, 250, [(host[i], host[-1], 2, start_time - 1000)])
    
    # for i in range(0, 3):
    #     # for j in range(1, 2):
    #         flows += flow.gen_flows(100, 2500000, [(host[i], host[-1], 2, start_time)])
...
def main() -> None:
    ...
    for CC in ['None']:
        for pg in [8]:
            ...
            #for mmu in ['DSH', 'Normal', 'DSHnoSH', 'DSHnoIH', 'Adaptive', 'DSHPLUS']:
            for mmu in ['Normal','DSH', 'Adaptive', 'DSHPLUS', 'Normal50', 'Normal80']:
                # for mmu in ['DH', 'DH3']:
                ks : List[Any]
                if mmu == 'Normal' or mmu == 'DSH' or mmu == 'QASH' or mmu == 'DSHnoSH' or mmu == 'DSHnoIH' or mmu == 'Adaptive' or mmu == 'DSHPLUS' or mmu == 'Normal50' or mmu == 'Normal80':
                    ks = [pg+1]
                else:
                    ks = [1, 1.5, 2, 2.5, 3]
                for k in ks:
                    ## hdrm_utilization_incast
                    test_name = exp_ins_test(
                        mx_pg=7,
                        cc=CC,
                        mmu_kind=mmu,
                        port_hdrm_k=k,
