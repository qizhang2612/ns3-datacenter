...
def exp_benchmark_isolation(  # pylint: disable=W0102
        nspine=16,
        nleaf=16,
        nhost=16,
        SL_Gbps=100,
    ...
    test_name = f'exp_benchmark_isolation-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg'
    ...
    copy_src = f'./data/exp_benchmark_isolation-search-xpod_Normal-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    copy_dst = f'./data/exp_benchmark_isolation-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    if mmu_kind != 'Normal' and os.path.exists(copy_src):
        ...
        for cos in [2, 3, 4, 5, 6, 7]:
            lam = (burst_load / 6) * (LH_Gbps * 1000 / 8) / flowsz.avg()
            if lam > 0:
                for src in hosts:
                    send_times = exp_send_time(lam, flow_num // (nhost * nleaf), seed=datetime.now().timestamp())
                    last_send_time = max(last_send_time, send_times[-1])
    ...
    with open(test_dir / 'config.txt', 'w') as configf:
        config.gen(test_name,
                   ...
                   stop_time=(start_time + 4000000) / 1e6,
                   pg_count=mx_pg+1,
                   sche_alg=sche,
                   quantums=quantums,
                   mmu_kind=mmu_kind,
            ...
            for pg in [8]:
                ...
                #for mmu in ['DSHnoIH']:
                    ...
                    if mmu == 'Normal' or mmu == 'DSH' or mmu == 'DSHnoSH' or mmu == 'DSHnoIH' or mmu == 'Adaptive' or mmu == 'DSHPLUS' or mmu == 'Normal50' or mmu == 'Normal80':
                        ...
                        ks = [pg+1]
                    for k in ks:
                        test_name = exp_benchmark_isolation(
                            cc=cc,
                            mx_pg=7,
                            mmu_kind=mmu,
                            port_hdrm_k=k,
