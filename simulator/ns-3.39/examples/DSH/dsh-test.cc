...
#include <ns3/switch-node.h>
#include <ns3/dsh-mmu.h>
#include <ns3/dshnosh-mmu.h>
#include <ns3/dshnoih-mmu.h>
#include <ns3/qash-mmu.h>
#include <ns3/dshplus-mmu.h>
...
bool enable_qcn = true, packet_level_ecmp = false, flow_level_ecmp = false;
uint32_t cc_mode = 1;
...
std::string packet_output_file = "packet.txt";
std::string buffer_output_file = "buffer.txt";
std::string drop_or_pause_file = "drop_or_pause.txt";
...
uint64_t buffer_mon_start = 2000000000, buffer_mon_interval = 100000;
string qlen_mon_file;
string thput_mon_file;
string ilen_mon_file;
string headroom_mon_file;

unordered_map<uint64_t, uint32_t> rate2kmax, rate2kmin;
unordered_map<uint64_t, double> rate2pmax;
SwitchNode::MmuKind mmu_kind = SwitchNode::DSH;
...
void qp_finish (FILE *fout, Ptr<RdmaQueuePair> q)
{
  ...
  fprintf (fout, "%08x %08x %u %u %u %lu %lu %lu\n", q->sip.Get (), q->dip.Get (), q->sport, q->dport,
           q->m_pg, q->m_size, q->startTime.GetTimeStep (),
           (Simulator::Now () - q->startTime).GetTimeStep ());
  fflush (fout);
}

void get_pfc (FILE *fout, Ptr<QbbNetDevice> dev, uint32_t type, unsigned pg)
{
  fprintf (fout, "%lu %u %u %u %u %u\n", Simulator::Now ().GetTimeStep (),
           dev->GetNode ()->GetId (), dev->GetNode ()->GetNodeType (), dev->GetIfIndex (), pg,
           type);
}

void get_packet(FILE *fout, Ptr<QbbNetDevice> dev, Ptr<const Packet>, Ptr<RdmaQueuePair>)
{
	fprintf (fout, "1 %lu %u %u %u\n", Simulator::Now ().GetTimeStep (),
           dev->GetNode ()->GetId (), dev->GetNode ()->GetNodeType (), dev->GetIfIndex ());
}
...
uint64_t thput_mon_start = 2000000000;
map<uint32_t, map<uint32_t, uint32_t>> throughput_result;
uint32_t thput_mon_interval = 20000;
void monitor_throughput (FILE *thput_output, NodeContainer *n)
{
  for (uint32_t i = 0; i < n->GetN (); i++)
    {
      if (n->Get (i)->GetNodeType () == 1)
        { // is switch
          Ptr<SwitchNode> sw = DynamicCast<SwitchNode> (n->Get (i));
          if (throughput_result.find (i) == throughput_result.end ())
            throughput_result[i];
          for (uint32_t j = 1; j < sw->GetNDevices (); j++)
            {
              throughput_result[i][j] = sw->m_mmu->RecordPortThroughput(j);
            }
        }
    }
  
  // dump
  for (auto &it0 : throughput_result)
    {
      fprintf (thput_output, "%lu %u", Simulator::Now ().GetTimeStep (), it0.first);
      for (auto &it1 : it0.second)
        {
          fprintf (thput_output, " %u", it1.first);
          fprintf (thput_output, " %.2f", 8.0*it1.second/thput_mon_interval/1.048576);
        }
      fprintf (thput_output, "\n");
      fflush (thput_output);
    }

    Simulator::Schedule (NanoSeconds (thput_mon_interval), &monitor_throughput, thput_output, n);
}

struct IQlenDistribution
{
  uint32_t lnum;
  uint32_t len[8];
  void add(uint32_t p,uint32_t l)
  {
    len[p] = l;
  }
};

map<uint32_t, map<uint32_t, IQlenDistribution>> iqueue_result;
void  monitor_ingress_queue(FILE *ilen_output, NodeContainer *n)
{
  fprintf (ilen_output, "0 %lu\n", Simulator::Now ().GetTimeStep ());
  for (uint32_t i = 0; i < n->GetN (); i++)
    {
      if (n->Get (i)->GetNodeType () == 1)
        { // is switch
          Ptr<SwitchNode> sw = DynamicCast<SwitchNode> (n->Get (i));
          if (iqueue_result.find (i) == iqueue_result.end ())
            iqueue_result[i];
          if (mmu_kind == SwitchNode::QASH)
            {
              Ptr<QashMmu> pfb_mmu = DynamicCast<QashMmu> (sw->m_mmu);
              fprintf (ilen_output, "1 %u %u\n", i, pfb_mmu->sharedXoff);
              fprintf (ilen_output, "4 %u %u\n", i, pfb_mmu->GetTotalHeadroom());
            }
          if (mmu_kind == SwitchNode::Normal)
            {
              Ptr<SwitchMmu> pfb_mmu = DynamicCast<SwitchMmu> (sw->m_mmu);
              fprintf (ilen_output, "4 %u %u\n", i, pfb_mmu->xoffTotal);
            }
          for (uint32_t j = 1; j < sw->GetNDevices (); j++)
            {
              for (uint32_t k = 0; k < SwitchMmu::qCnt; k++)
                iqueue_result[i][j].add (k, sw->m_mmu->ingress_bytes[j][k]);
            }
        }
    }
  for (auto &it0 :iqueue_result)
    for (auto &it1 : it0.second)
      {
        fprintf (ilen_output, "2 %u %u", it0.first, it1.first);
        auto &dist = it1.second.len;
        for (uint32_t i = 0; i < SwitchMmu::qCnt; i ++)
        {
          fprintf (ilen_output, " %u", dist[i]);
          dist[i] = 0;
        }
        fprintf (ilen_output, "\n");
      }
  fflush (ilen_output);
  //if (Simulator::Now ().GetTimeStep () < qlen_mon_end)
  Simulator::Schedule (NanoSeconds (qlen_mon_interval), &monitor_ingress_queue, ilen_output, n);
}

// map<uint32_t, double> buffer_result;
// void monitor_buffer_use (FILE *bu_output, NodeContainer *n)
// {
// 	for (uint32_t i = 0; i < n->GetN (); i++)
// 	{
// 		if (n->Get (i)->GetNodeType () == 1)
// 		{ // is switch
// 			Ptr<SwitchNode> sw = DynamicCast<SwitchNode> (n->Get (i));
// 			buffer_result[i] = sw->m_mmu->RecordBufferUse();
// 		}
// 	}
  
//   // dump
// 	fprintf (bu_output, "0 %lu\n", Simulator::Now ().GetTimeStep ());
// 	for (auto &it0 : buffer_result)
// 	{
// 		fprintf (bu_output, "1 %u %.2f\n", it0.first, it0.second);
// 		fflush (bu_output);
// 	}
// 	Simulator::Schedule (NanoSeconds (buffer_mon_interval), &monitor_buffer_use, bu_output, n);
// }
...
int main(int argc, char *argv[])
{
	...
	uint32_t mmuKind = 1;
	std::string confFile = "/home/laozhang/ns3-datacenter/simulator/ns-3.39/examples/DSH/data/exp_collateral_damage_DSH-DWRR-HPCC-25h-8pg/config.txt";
	CommandLine cmd;
	...
	while (!conf.eof())
	{
		...
		} else if(key.compare("PACKET_OUTPUT_FILE") == 0)
		{
			conf >> packet_output_file;
			std::cout << "PACKET_OUTPUT_FILE\t\t\t\t" << packet_output_file << '\n';
		} 
		...
		}else if (key.compare ("BUFFER_OUTPUT_FILE") == 0){
			conf >> buffer_output_file;
			std::cout << "BUFFER_OUTPUT_FILE\t\t\t\t" << buffer_output_file << '\n';
		}else if (key.compare ("DROP_OR_PAUSE_FILE") == 0){
			conf >> drop_or_pause_file;
			std::cout << "DROP_OR_PAUSE_FILE\t\t\t\t" << drop_or_pause_file << '\n';
		}else if (key.compare("QLEN_MON_FILE") == 0) {
			conf >> qlen_mon_file;
			std::cout << "QLEN_MON_FILE\t\t\t\t" << qlen_mon_file << '\n';
		}else if (key.compare ("THPUT_MON_FILE") == 0){
			conf >> thput_mon_file;
			std::cout << "THPUT_MON_FILE\t\t\t\t" << thput_mon_file << '\n';              
        } 
		else if (key.compare("ILEN_MON_FILE") == 0)
		{
			conf >> ilen_mon_file;
			std::cout << "ILEN_MON_FILE\t\t\t\t" << ilen_mon_file << '\n';
		}
		else if (key.compare("HEADROOM_MON_FILE") == 0)
		{
			conf >> headroom_mon_file;
			std::cout << "HEADROOM_MON_FILE\t\t\t\t" << headroom_mon_file << '\n';
		}
		...
		}else if (key.compare ("WIEN") == 0)
		{
          ...
          else if (key.compare ("DELAY_WIEN") == 0)
		{
			bool v;
			conf >> v;
			delayWien = v;
			std::cout << "DELAY_WIEN\t\t\t\t" << delayWien << '\n';
		} 
	...
	{
		mmu_kind = SwitchNode::DSH;
		std::cout << "mmu:DSH" << std::endl;
	}
	else if(mmuKind == 3)
	{
		mmu_kind = SwitchNode::DSHnoSH;
		std::cout << "mmu:DSHnoSH" << std::endl;
	}
	if(mmuKind == 4)
	{
		mmu_kind = SwitchNode::DSHnoIH;
		std::cout << "mmu:DSHnoIH" << std::endl;
	}
	if(mmuKind == 5)
	{
		mmu_kind = SwitchNode::QASH;
		std::cout << "mmu:QASH" << std::endl;
	}
	if(mmuKind == 6)
	{
		mmu_kind = SwitchNode::DSHPLUS;
		std::cout << "mmu:DSHPLUS" << std::endl;
	}
	if(mmuKind == 7)
	{
		mmu_kind = SwitchNode::Adaptive;
		std::cout << "mmu:Adaptive" << std::endl;
	}
	if(mmuKind == 8)
	{
		mmu_kind = SwitchNode::Normal50;
		std::cout << "mmu:Normal50" << std::endl;
	}
	if(mmuKind == 9)
	{
		mmu_kind = SwitchNode::Normal80;
		std::cout << "mmu:Normal80" << std::endl;
	}
	...
	FILE *packet_file = fopen(packet_output_file.c_str(), "w");
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (node_type[i] == 0) {
			...
			Ptr<SwitchNode> sw = CreateObject<SwitchNode>(mmu_kind);
			n.Add(sw);
			switchNodes.Add(sw);
			allNodes.Add(sw);
	...
	for (uint32_t i = 0; i < link_num; i++)
	{
		...
		// setup packet trace
		if(packet_file != nullptr)
		{
			if(!snode->GetNodeType())
			{
				DynamicCast<QbbNetDevice>(d.Get(0))->TraceConnectWithoutContext("RdmaQpDequeue", MakeBoundCallback (&get_packet, packet_file, DynamicCast<QbbNetDevice>(d.Get(0))));
			}
			if(!dnode->GetNodeType())
			{
				DynamicCast<QbbNetDevice>(d.Get(1))->TraceConnectWithoutContext("RdmaQpDequeue", MakeBoundCallback (&get_packet, packet_file, DynamicCast<QbbNetDevice>(d.Get(1))));
			}
			// DynamicCast<QbbNetDevice>(d.Get(0))->TraceConnectWithoutContext("QbbDrop", MakeBoundCallback (&get_drop, packet_file, DynamicCast<QbbNetDevice>(d.Get(0))));
			// DynamicCast<QbbNetDevice>(d.Get(1))->TraceConnectWithoutContext("QbbDrop", MakeBoundCallback (&get_drop, packet_file, DynamicCast<QbbNetDevice>(d.Get(0))));
		}
		
	}

	nic_rate = get_nic_rate(n);
	FILE *drop_output = fopen (drop_or_pause_file.c_str (), "w");
	FILE *buffer_output = fopen (buffer_output_file.c_str (), "w");
	FILE *headroom_output = fopen (headroom_mon_file.c_str(), "w");
	// config switch
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType()) { // is switch
			...
			double alpha = 1.0 / 16;
			sw->m_mmu->SetDropFile(drop_output);
			sw->m_mmu->SetBufferFile(buffer_output);
			sw->m_mmu->SetHeadroomFile(headroom_output);
			sw->m_mmu->SetPortCount(32);
			sw->m_mmu->SetAlphaIngress(alpha);
			sw->m_mmu->SetAlphaEgress(UINT16_MAX);
			sw->m_mmu->SetReserved(3072, "ingress");
			if(mmu_kind == SwitchNode::Adaptive)
			{
				uint32_t headroom = 57020;
				sw->m_mmu->SetHeadroom(headroom);
			}
			else if(mmu_kind == SwitchNode::Normal)
			{
				uint32_t headroom = 60000;
				sw->m_mmu->SetHeadroom(headroom);
			}
			else if(mmu_kind == SwitchNode::Normal50)
			{
				uint32_t headroom = 57020 * 0.5;
				sw->m_mmu->SetHeadroom(headroom);
			}
			else if(mmu_kind == SwitchNode::Normal80)
			{
				uint32_t headroom = 57020 * 0.8;
				sw->m_mmu->SetHeadroom(headroom);
			}
			else if(sw->m_mmu_kind == SwitchNode::DSH || sw->m_mmu_kind == SwitchNode::DSHnoSH || sw->m_mmu_kind == SwitchNode::DSHPLUS)
			{
				uint32_t headroom = 60000;
				sw->m_mmu->SetHeadroom(headroom);
			}
			else if(sw->m_mmu_kind == SwitchNode::DSHnoIH)
			{
				uint32_t headroom = 0;
				sw->m_mmu->SetHeadroom(headroom);
			}
			...
			for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
				...
				// headroom =
                //     (rate * delay / 8 / 1000000000 + (packet_payload_size + 90)) * 2 + 
                //     (60 * 512 / 8) + (packet_payload_size + 90);
				headroom =
                    (rate * delay / 8 / 1000000000 + (packet_payload_size + 90)) * 2 + 
                    (60 * 512 / 8);
					
				if(mmu_kind == SwitchNode::Adaptive && headroom == 57020)
				{
					headroom = 57095;
				}
				...
				{
					headroom = 60000;
					Ptr<DshMmu> mmu = DynamicCast<DshMmu> (sw->m_mmu);
					mmu->SetHeadroom(headroom, j);
					mmu->SetAlphaIngress(alpha, j);
					//totalHeadroom += headroom;
				}
				else if(sw->m_mmu_kind == SwitchNode::DSHnoIH)
				{
				...
				else if(sw->m_mmu_kind == SwitchNode::QASH)
				{
					Ptr<QashMmu> mmu = DynamicCast<QashMmu> (sw->m_mmu);
					mmu->SetHeadroom(headroom, j);
					mmu->SetAlphaIngress(alpha, j);
					//totalHeadroom += headroom;
				}
				else if(sw->m_mmu_kind == SwitchNode::Normal50)
				{
					for (uint32_t qu = 1; qu < 8; qu++) {
						sw->m_mmu->SetHeadroom(headroom * 0.5, j, qu);
						//totalHeadroom += headroom * 0.5;
					}
				}
				else if(sw->m_mmu_kind == SwitchNode::Normal80)
				{
					for (uint32_t qu = 1; qu < 8; qu++) {
						sw->m_mmu->SetHeadroom(headroom * 0.8, j, qu);
						//totalHeadroom += headroom * 0.8;
					}
				}
			}
			...
			// sw->m_mmu->PrintInfo();
			Simulator::Schedule(Seconds(1.01), &SwitchMmu::PrintInfo, sw->m_mmu);
		}
	}
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType() == 0) { // is server
			...
			rdmaHw->SetAttribute("NACKGenerationInterval", DoubleValue(32));
	...
	for (uint32_t i = 0; i < flow_num; i++)
    {
	  ...
	  RdmaClientHelper clientHelper(pg, serverAddress[src], serverAddress[dst], port, dport, maxPacketCount, has_win ? (global_t == 1 ? maxBdp : pairBdp[n.Get(src)][n.Get(dst)]) : 0, global_t == 1 ? maxRtt : pairRtt[src][dst], Simulator::GetMaximumSimulationTime());
      ApplicationContainer appCon = clientHelper.Install (n.Get (src));
      appCon.Start (NanoSeconds (start_time * 1000));
    }
	...
	FILE *thput_output = fopen (thput_mon_file.c_str (), "w");
	//FILE *ilen_output = fopen (ilen_mon_file.c_str (), "w");
	if (thput_output != nullptr)
	{
		Simulator::Schedule (NanoSeconds (thput_mon_start), &monitor_throughput, thput_output, &n);
	}
	// if(ilen_output != nullptr)
	// {
	// 	Simulator::Schedule (NanoSeconds (qlen_mon_start), &monitor_ingress_queue, ilen_output, &n);
	// }
