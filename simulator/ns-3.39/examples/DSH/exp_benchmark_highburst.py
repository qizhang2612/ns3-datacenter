...
def exp_benchmark_higburst(  # pylint: disable=W0102
        nspine=16,
        nleaf=16,
        nhost=16,
        SL_Gbps=100,
    ...
    test_name = f'exp_benchmark_highburst-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_hosts}bursthosts-{burst_size}KB-{mx_pg+1}pg'
    ...
    copy_src = f'./data/exp_benchmark_highburst-search-xpod_Normal-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_hosts}bursthosts-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    copy_dst = f'./data/exp_benchmark_highburst-search-xpod_{mmu_kind}-{sche}-{cc}-{back_load}back-{burst_load}burst-{burst_hosts}bursthosts-{burst_size}KB-{mx_pg+1}pg/flow.txt'
    if mmu_kind != 'Normal' and os.path.exists(copy_src):
        print(copy_src + "exist")
                ...
                for pg in [8]:
                    ...
                    for mmu in ['DSHPLUS']:
                        ...
                        if mmu == 'Normal' or mmu == 'DSH' or mmu == 'DSHnoSH' or mmu == 'DSHnoIH' or mmu == 'Adaptive' or mmu == 'DSHPLUS' or mmu == 'Normal50' or mmu == 'Normal80':
                            ...
                            ks = [pg+1]
                        for k in ks:
                            test_name = exp_benchmark_higburst(
                                cc=cc,
                                mx_pg=7,
                                mmu_kind=mmu,
                                port_hdrm_k=k,
